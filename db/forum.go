...
LEFT JOIN profile p ON (p.uid = f.author_id) WHERE `
	...
	if isUid {
		...
		sqlStatement1 += `f.slug = $1 GROUP BY f.uid, f.title;`
		sqlStatement2 += `f.slug = $1 GROUP BY f.uid, p.nickname;`
		...
		fmt.Println(forum)
		if err != nil {
			return Forum{Slug: slug}, errors.New("Can't find forum with slug: " + slug)
		}
...
SELECT * FROM (
	...
	fmt.Println(sqlStatement, forumId, limit)
